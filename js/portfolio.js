//These functions open and clsoe the contact form
function openForm(){
...
//this function cahnges the slide when the left or right arrows are clicked
function plusSlides(n){
    showSlides(slideIndex += n);
}

//this function chagnes the slide when the dots are clicked
...
function showSlides(n){
    ...
    var dots = document.getElementsByClassName("dot");//Thist akes all elements with the class name "dot" ands tores them in the variable array "dots"
    if (n > slides.length) {slideIndex = 1}; // If n (the number passsed into the function) is greater than the length of the array "slides", the slideIndex is set to 1
    if (n < 1) {slideIndex = slides.length}; // If n (the number passed into the fnction) is less than 1, the slideIndex is set to the length of the array "slides"
